...
/* ------------------------- Chart export function -------------------------
   Generates a static SVG for the chart, suitable for PNG export:
   • disables labels and interactive elements
...
export function generateChartSVG({
    ...
    height = 1080,
    subtitle = "",
    exportMode = false // if true, disables labels, adjusts styling for PNG
}) {
    ...
    const yMax = d3.max(data, d => d.Production / meta.scale);
    ...
    // === Base (grey) lines ===
    const baseGroups = svg.selectAll("g.base-line").data(baseLines, d => d.id);
    const baseEnter = baseGroups.enter().append("g").attr("class", "base-line");
    ...
    const guessGroups = svg.selectAll("g.guess-line").data(guessLines, d => d.id);
    const guessEnter = guessGroups.enter().append("g").attr("class", "guess-line");
    ...
    let gTarget = svg.select("g.target-line");
    if (gTarget.empty()) {
        gTarget = svg.append("g").attr("class", "target-line");
